...
module L1Cache_SA (
    ...
    // DATAMEMORY SIDE (128-bit data transfer)
    ...
    // On access: set lru_way to the OTHER way (the one we just used is now MRU, other is now LRU)
    ...
    // mem_wait_seen   : DataMemory confirmed busy at least once prevents false early completion
    ...
    always @(posedge clk) begin
        if (~rst) begin
            ...
            case (state)
                ...
                // Sets hit_way = victim_way so UPDATE knows where the fresh data is.
                FILL: begin
                    if (!mem_req_pending) begin
                ...
                // Delay of 1 cycle before IDLE so PC advances cleanly
