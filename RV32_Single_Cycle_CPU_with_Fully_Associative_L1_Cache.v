...
// DATA MEMORY INTERFACING — CPU → L1Cache_FA → DataMemory

wire [31:0] memReadData;  // load result → writeback mux
wire mem_wait;     // → PC and Register file

// 128-bit wires between L1Cache_FA and DataMemory - For 'Block' Transfer !!
wire [31:0]  dm_addr;
...
module L1Cache_FA (
    ...
    // CACHE STORAGE — 8 lines x 4 words = 128 bytes total
    reg valid [0:7];
    reg dirty [0:7];
    reg [27:0] tag_store [0:7];   // 28-bit tag (no index in FA cache)
    reg [31:0] cache_data[0:7][0:3];

    // LRU AGES — 3 bits per cache line (as number of cache lins are 8)
    // 7 = most recently used, 0 = least recently used (evict this when cache is full)
    reg [2:0] lru_age [0:7];
    ...
    // LATCHED CPU MEMORY REQUEST INFORMATION
    reg [31:0] lat_addr;
    reg [31:0] lat_writeData;
    reg lat_memRead;
    reg lat_memWrite;

    // ADDRESS DECOMPOSITION — No index field in fully associative
    // | 31 ............. 4 | 3   2 | 1   0 |
    // |     tag  (28 bits) | word  | byte  |
    wire [1:0]  word_offset = lat_addr[3:2];
    wire [27:0] tag         = lat_addr[31:4];

    // REGISTERED RESULTS FROM TAG_CHECK
    reg hit_found;
    reg [2:0] hit_line;
    reg [2:0] victim_line;

    // LOCAL VARIABLES FOR TAG_CHECK RESULT - HIT OR MISS ??
    reg loc_hit_found;
    reg [2:0]  loc_hit_line;
    reg [2:0]  loc_victim_line;

    // mem_req_pending : request has been issued to DataMemory
    // mem_wait_seen   : DataMemory has gone busy at least once
    //                   (prevents false-early-completion) - only once mem_wait_seen = 0 will completion be considered done
    reg mem_req_pending;
    reg mem_wait_seen;
    ...
    // mem_wait — combinational
    // Asserts same cycle CPU issues request so PC cannot advance
    assign mem_wait = (state == IDLE) ? (cpu_memRead | cpu_memWrite) : (state != DONE);

    integer i;

    // LRU UPDATE TASK
    // Called after every cache access (hit or miss fill)
    // accessed_line just became most recently used → age = 7
    // All lines with higher age than accessed line → decrement by 1
    task update_lru;
        input [2:0] accessed_line;
        integer j;
        reg [2:0] old_age;
        begin
            old_age = lru_age[accessed_line];
            for (j = 0; j < 8; j = j + 1) begin
                if (j != accessed_line && lru_age[j] > old_age)
                    lru_age[j] <= lru_age[j] - 1;
            end
            lru_age[accessed_line] <= 3'd7;
        end
    endtask
    ...
    always @(posedge clk) begin
        if (~rst) begin
            ...
            for (i = 0; i < 8; i = i + 1) begin
                ...
                dirty[i] <= 0;
                tag_store[i] <= 0;
                lru_age[i] <= i[2:0]; // distinct initial ages 0..7
            end
        end
        else begin
            case (state)
                // IDLE — wait for CPU request, latch upon recival 
                IDLE: begin
                    dm_memRead <= 0;
                    dm_memWrite <= 0;
                    mem_req_pending <= 0;
                ...
                TAG_CHECK: begin
                    ...
                    //  initialise local signals 
                    loc_hit_found = 1'b0;
                    loc_hit_line = 3'd0;
                    loc_victim_line = 3'd0;

                    //  parallel tag search
                    for (i = 0; i < 8; i = i + 1) begin
                        if (valid[i] && (tag_store[i] == tag)) begin
                            loc_hit_found = 1'b1;   // immediate
                            loc_hit_line  = i[2:0]; // immediate
                        end
                    end
                    ...
                    for (i = 0; i < 8; i = i + 1) begin
                        if (lru_age[i] == 3'd0)
                            loc_victim_line = i[2:0]; // immediate
                    end

                    // latch results into FSM registers
                    hit_found <= loc_hit_found;
                    hit_line <= loc_hit_line;
                    victim_line <= loc_victim_line;

                    // state decision using LOCAL signals
                
                    if (loc_hit_found) begin
                        hit_count <= hit_count + 1;
                        state <= UPDATE;
                        // hit_line will be loc_hit_line (latched above)
                    end
                    else begin
                        miss_count <= miss_count + 1;
                        mem_req_pending <= 0;
                        mem_wait_seen <= 0;
                        // victim_line will be loc_victim_line (latched above)
                        if (valid[loc_victim_line] && dirty[loc_victim_line])
                            state <= WRITEBACK;
                        else
                            state <= FILL;
                    end
                ...
                // WRITEBACK
                // Flush dirty victim line to DataMemory.
                // Address reconstructed from tag_store[victim_line].
                //   Step 1: issue burst write request
                //   Step 2: wait until dm_mem_wait goes HIGH
                //   Step 3: wait until dm_mem_wait goes LOW → done
                
                WRITEBACK: begin
                    if (!mem_req_pending) begin
                        // STEP 1 — Issue burst write
                        dm_addr <= {tag_store[victim_line], 4'b0000};
                        dm_writeData <= {cache_data[victim_line][3],
                                         cache_data[victim_line][2],
                                         cache_data[victim_line][1],
                                         cache_data[victim_line][0]};
                        dm_memWrite <= 1;
                        dm_memRead <= 0;
                        mem_req_pending <= 1;
                        mem_wait_seen <= 0;
                        ...
                        // STEP 3 — Complete after busy→idle transition
                ...
                // FILL
                // Load new block into victim_line from DataMemory.
                // Same three-step handshake as WRITEBACK.
                // Sets hit_line = victim_line so UPDATE knows where
                // the fresh data went
                FILL: begin
                    if (!mem_req_pending) begin
                        ...
                        // STEP 3 — Complete after busy→idle transition
                        if (mem_wait_seen && !dm_mem_wait) begin
                            cache_data[victim_line][0] <= dm_readData[31:0];
                            cache_data[victim_line][1] <= dm_readData[63:32];
                            cache_data[victim_line][2] <= dm_readData[95:64];
                            cache_data[victim_line][3] <= dm_readData[127:96];
                            valid[victim_line] <= 1;
                            dirty[victim_line] <= 0;
                            tag_store[victim_line] <= tag;
                            dm_memRead  <= 0;
                            mem_req_pending <= 0;
                            mem_wait_seen <= 0;
                            // point hit_line at the freshly filled line
                            hit_line <= victim_line;
                            state <= UPDATE;
                        end
                    end
                end
                ...
                // UPDATE (1 cycle)
                // READ:  forward word to CPU
                // WRITE: write into cache, mark dirty (write-back policy)
                // Always update LRU after any access
            
                UPDATE: begin
                    if (lat_memRead)
                        cpu_readData <= cache_data[hit_line][word_offset];
                    else if (lat_memWrite) begin
                        cache_data[hit_line][word_offset] <= lat_writeData;
                        dirty[hit_line] <= 1;
                    end
                    update_lru(hit_line);
                    state <= DONE;
                ...
                // Chill one cycle before returning to idle - for timing purposes amigo
                DONE: begin
                    state <= IDLE;
                end
            endcase
